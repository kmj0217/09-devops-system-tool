...
- 应能够实现通过各类告警采集接口，如：Syslog、Trap、文件、数据库等，完成统一告警数据采集任务。
...
- 拓扑信息浏览：提供分层次的拓扑展示方式，上层拓扑可以向下钻取，可以设置下层设备告警是否向上层传递以及传递规则。可根据运维人员给定的条件（如：按IP地址、节点名称、厂家等条件）方便地在多个拓扑视图中进行网元的快速查询和定位。系统提供拓扑图的缩放功能，便于查看拓扑的局部信息。拓扑图间可互相引用，可在拓扑间进行灵活的视图切换。
