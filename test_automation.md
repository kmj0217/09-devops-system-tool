...
——支持多种报文格式，不限于表单、文本、JSON、XML等

——支持接口鉴权认证，不限于Basic认证、Token认证和CA证书等方式

——支持针对报文响应的响应报文、响应头、响应码定义测试断言

——支持提取报文响应的内容并基于上下文创建后续请求报文

——支持测试流程逻辑控制，不限于循环、判断、暂停等
...
——支持基础性能指标采集，包括但不限于安装时间、启动时间、CPU使用率、内存占用量、流量、和FPS等
...
——支持基础性能指标采集，包括安装时间、首屏时间、响应时间（启动、滑动、界面切换等）、CPU、内存、流量、FPS(Frames Per Second）、耗电量、温度、I/O等客户端必要的指标
...
——支持性能测试指标数据实时采集、保存、分析和展示；

——支持性能测试报告的生成、查看和导出；

——支持性能测试过程中错误信息的分类显示、聚合；
