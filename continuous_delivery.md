...
——构建环境调度需支持主流操作系统，如Windows、Linux、macOS等；
...
——构建结果可设定多种通知方式，支持电子邮件、短信、电话以及即时通讯等；
...
——支持配置编排流水线，可按需定制阶段/步骤/任务

——支持把其他服务作为流水线任务，如代码拉取、代码检查、编译构建、部署、测试、发布等

——支持跳转其他服务查看任务详情

——支持手动、定时、版本控制系统事件等多种触发方式；
...
——支持数据统计，如流水线总数、成功率、健康度等

——支持流水线中的任务自动流转、手动流转

——支持查看以及下载流水线的产出物

——支持流水线质量管理，如质量门禁、流水线及任务的健康度评估等

——支持流水线内容及结果的通知，如即时消息、电子邮件、短信服务等
...
——支持查看流水线的一次运行所对应的当时的流水线配置和参数

——支持保存流水线执行中的产出物，如测试报告、测试覆盖率报告等
...
——支持流水线配置复用，比如通过抽取公共代码并复用

——支持每条分支按照其上的流水线配置代码自动创建该分支的流水线
...
——发布定义，发布定义不限于制定发布审批工作流、确定发布通知策略等

——发布策略，发布策略不限于滚动发布、蓝绿发布、金丝雀发布等
...
环境是指应用安装和运行所需要的资源及其配置。在一定的基础资源（如 IaaS 或 PaaS）之上，环境管理工具可以用以描述资源配置信息，为开发、测试、生产等场景提供环境支撑。
...
应用配置管理是指应用启动或运行时影响应用行为的配置项的管理，如静态文件的配置管理、基于键值对的模板配置管理等。应用配置管理让应用的配置变更、发布过程与程序的开发、发布过程解耦，使应用程序能够在启动或运行时响应配置的变更，提升交付效率。

##### 应包含以下基本功能：

——支持配置的最小粒度对应唯一标识，其内容如KV，Json, XML等格式，且支持自由配置

——支持配置内容大小的限制

——支持分组隔离，如按用户组、环境、应用等分组隔离
...
——提供多种编程语言的SDK来操作配置的功能。

——支持通过推送或者订阅等方式实现配置的热更新。

——支持配置的灰度发布，如根据某类对象的IP或标签等。

——支持对配置订阅内容的监控，能查看配置被订阅的所有对象

### 9. 数据变更管理

数据变更管理是指在应用的测试阶段和生产部署流水线过程中，对测试及生产数据进行自动化、规范化的组织管理，确保在测试及部署阶段数据安全性、可靠性及管理时效的提升。
...
——所有变更都有操作历史可供审计，如提供日志功能，日志可包括数据查询、变更、备份等操作记录

##### 可以包含以下高级功能：

——数据库变更审批流程自动化；

——支持数据库变更的自动化执行（无须DBA参与）

——对数据的变更，支持不同格式数据的导入，支持按照特定规则自动生成数据（来源：支持按照指定规则自动创建）；

——支持变更后的数据库性能监控，如慢查询监控；

——支持变更的回滚操作，如自动生成回滚语句，或回滚到之前的数据库版本；
